...
rain_summary <- mrms_metrics %>%
  filter(between(endtime, ymd_hms('2022-06-01 00:00:00'),
                 ymd_hms('2022-10-01 00:00:00'))) %>% 
  ungroup() %>% 
...
# Density for MI60
ggplot(mrms_metrics, aes(x = MI60)) +
  geom_density() +
  facet_wrap(~site)

ggplot(mrms_metrics, aes(x = MI60, color = site)) +
 stat_ecdf(geom = "step") +
  # facet_wrap(~site) +
  xlim(0,10)
...
p1_response2$pred <- predict(p1_glm, p1_response2, type = 'response')
...
p2_response2$pred <- predict(p2_glm, p2_response2, type = 'response')
...
hm_response2$pred <- predict(hm_glm, hm_response2, type = 'response')
...
hum_response2$pred <- predict(hum_glm, hum_response2, type = 'response')
...
mum_response2$pred <- predict(mum_glm, mum_response2, type = 'response')
...
mpm_response2$pred <- predict(mpm_glm, mpm_response2, type = 'response')
...
mub_response2$pred <- predict(mub_glm, mub_response2, type = 'response')
